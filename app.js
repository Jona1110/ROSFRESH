...
function setupEventListeners() {
    ...
    // Evento para opciones de entrega a domicilio
...
function renderGrid(products) {
    ...
    products.forEach(p => {
        const imageHTML = p.imagen && p.imagen.trim() !== "" 
            ? `<div class="product-image-container"><img src="${p.imagen}" alt="${p.nombre}"></div>` 
            : '';

        const isAgotado = p.estado === 'Agotado';
        const cardClass = isAgotado ? 'product-card agotado' : 'product-card';
        const agotadoBadge = isAgotado ? `<div class="badge-agotado">Agotado</div>` : '';
        const btnHTML = isAgotado 
            ? `<button class="btn-add" style="background:#b2bec3; cursor:not-allowed;"><i class="fas fa-ban"></i> Agotado</button>`
            : `<button class="btn-add" onclick="openCustomModal('${p.id}', '${p.nombre}', ${p.precio}, '${p.categoria}')"><i class="fas fa-plus"></i> Agregar</button>`;
...
function addToCart(id, name, price, salsas, toppings) {
    ...
    if (existingItem) {
        ...
        cart.push({ cartKey, id, name, price: parseFloat(price), salsas, toppings, qty: 1 });
    }
    saveAndRenderCart();
...
// --- ENVIAR PEDIDO A WHATSAPP Y REGISTRAR EN FINANZAS ---
async function sendWhatsAppOrder() {
    ...
    orderDetailsText = orderDetailsText.slice(0, -2);
    ...
    try {
        const payload = {
            ...
            tipo: "Ingreso",
            monto: total,
            detalle: `Pedido Web: ${orderDetailsText} | Envío: ${deliveryType}`
        };
